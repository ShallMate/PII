...
void testrun(int argc, const char** argv)
{
    ez::ezOptionParser opt;
    IbsOptions opts(opt, argc, argv);
    opt.add(
            "", // Default.
            0, // Required?
    ...
    vector<PIIIBSInput> piiinputs;
    vector<PIIIBSInput> generatedinputsA;
    vector<PIIIBSInput> generatedinputsB;
    ClearInput<PIIIBSInput> clearInput(P);
    ...
    if (P.my_num() == 0){
        ...
        for (int i = 0; i < INPUTSIZE; i++){
            piiinputs.push_back(generatedinputsA[i]);
            cout << piiinputs[i].Pk << piiinputs[i].signature << endl;
        }

        // free unused vector
        vector<PIIIBSInput>().swap(generatedinputsA);
        ...
        // free unused vector
        vector<PIIIBSInput>().swap(generatedinputsB);
    ...
    if (P.my_num() == 1){
        ...
        for (int i = 0; i < INPUTSIZE; i++)
        {
            piiinputs.push_back(clearInput.finalize(0));
            cout << piiinputs[i].Pk << piiinputs[i].signature << endl;
        }
    }
    ...
    cout <<  "------ Input /sigma_{i,1}, /sigma_{i,2}, ID_i, m_i ----------" << endl;
    ...
    for (int i = 0; i < INPUTSIZE; i++){
        g1_input.add_from_all(piiinputs[i].signature);
        g2_input.add_from_all(piiinputs[i].Pk);
    ...
    vector<PIIIBSInput>().swap(piiinputs);
    ...
    // cout <<  "------inputs /sigma_{i,1}, /sigma_{i,2}, ID_i shared ----------" << endl;
    ...
    cout << "---- computing u1j and u2j ----" << N.my_num() << endl;
    ...
    cout << ">>>> u1j u2j computation," << (tc1c2 - tinput) * 1e3 << ", ms" << endl;
    ...
    cout << ">>>> generate randoms," << (topen3rand - tc1c2) * 1e3 << ", ms" << endl;


    cout << "------  generate " << INPUTSIZE << " randoms ------" << endl;
    ...
    cout << ">>>> v_{i,j} computation," << (tc3 - tprand) * 1e3 << ", ms" << endl;

    cout << "-- computing w_{i,j} --" << endl;
    ...
    cout << ">>>> k_{i,j} computation," << (tc4_rand - tc4) * 1e3 << ", ms" << endl;


    // Test
    cout << "-- opening k_{i,j} --" << endl;
    ...
    cout << ">>>> Open k_{i,j}," << (tc4_open - tc4_rand) * 1e3 << ", ms" << endl;
